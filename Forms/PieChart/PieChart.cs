...
namespace SummerGUI.Charting.PieChart
{
	...
	public class PieChart3D	: IDisposable {
        ...
        /// </summary>
		protected PieChart3D(IGUIContext ctx) {
			CTX = ctx;
			m_font = CTX.FontManager.DefaultFont;
		}
        ...
        /// </param>
        public PieChart3D(IGUIContext ctx, float xBoundingRect, float yBoundingRect, float widthBoundingRect, float heightBoundingRect, decimal[] values) 
			: this(ctx) 
		{
        ...
        ///   bounding rectangle.
        /// </param>
        public PieChart3D(IGUIContext ctx, float xBoundingRect, float yBoundingRect, float widthBoundingRect, float heightBoundingRect, decimal[] values, float sliceRelativeHeight) 
            : this(ctx, xBoundingRect, yBoundingRect, widthBoundingRect, heightBoundingRect, values) {
        ...
        public PieChart3D(IGUIContext ctx, RectangleF boundingRectangle, decimal[] values, float sliceRelativeHeight)
            : this(ctx, boundingRectangle.X, boundingRectangle.Y, boundingRectangle.Width, boundingRectangle.Height, values, sliceRelativeHeight) {
        }
        ...
        ///   bounding rectangle.
        /// </param>
		public PieChart3D(IGUIContext ctx, float xBoundingRect, float yBoundingRect, float widthBoundingRect, float heightBoundingRect, decimal[] values, Color[] sliceColors, float sliceRelativeHeight)
            : this(ctx, xBoundingRect, yBoundingRect, widthBoundingRect, heightBoundingRect, values, sliceRelativeHeight) {
        ...
        /// </param>
		public PieChart3D(IGUIContext ctx, RectangleF boundingRectangle, decimal[] values, Color[] sliceColors, float sliceRelativeHeight)
            : this(ctx, boundingRectangle, values, sliceRelativeHeight) {
            m_sliceColors = sliceColors;
        }
        ...
        /// </param>
		public PieChart3D(IGUIContext ctx, float xBoundingRect, float yBoundingRect, float widthBoundingRect, float heightBoundingRect, decimal[] values, Color[] sliceColors, float sliceRelativeHeight, string[] texts)
            : this(ctx, xBoundingRect, yBoundingRect, widthBoundingRect, heightBoundingRect, values, sliceColors, sliceRelativeHeight) {
            m_texts = texts;
        }
        ...
        ///   An array of strings that are displayed on corresponding slice.
        /// </param>
		public PieChart3D(IGUIContext ctx, float xBoundingRect, float yBoundingRect, float widthBoundingRect, float heightBoundingRect, decimal[] values, float sliceRelativeHeight, string[] texts) 
            : this(ctx, xBoundingRect, yBoundingRect, widthBoundingRect, heightBoundingRect, values, sliceRelativeHeight) {
            m_texts = texts;
        }
		...
		public void Draw(IGUIContext graphics, Rectangle bounds) {
            ...
            if (m_sliceRelativeHeight > float.Epsilon) {
                DrawSliceSides(graphics, bounds);
            }
            DrawTops(graphics);
        }
        ...
        protected virtual PieSlice CreatePieSlice(float boundingRectLeft, float boundingRectTop, float boundingRectWidth, float boundingRectHeight, float sliceHeight, float startAngle, float sweepAngle, Color color, ShadowStyle shadowStyle, EdgeColorType edgeColorType, float edgeLineWidth) {
            return new PieSlice(CTX, boundingRectLeft, boundingRectTop, boundingRectWidth, boundingRectHeight, sliceHeight, (float)(startAngle % 360), sweepAngle, color, shadowStyle, edgeColorType, edgeLineWidth);
        }
        ...
        protected virtual PieSlice CreatePieSliceHighlighted(float boundingRectLeft, float boundingRectTop, float boundingRectWidth, float boundingRectHeight, float sliceHeight, float startAngle, float sweepAngle, Color color, ShadowStyle shadowStyle, EdgeColorType edgeColorType, float edgeLineWidth) {
            ...
            return new PieSlice(CTX, boundingRectLeft, boundingRectTop, boundingRectWidth, boundingRectHeight, sliceHeight, (float)(startAngle % 360), sweepAngle, highLightedColor, shadowStyle, edgeColorType, edgeLineWidth);
        }
        ...
        protected SizeF GetSliceDisplacement(float angle, float displacementFactor) {
            ...
            if (displacementFactor < float.Epsilon)
                return SizeF.Empty;
        ...
        /// </param>
		protected void DrawSliceSides(IGUIContext graphics, Rectangle bounds) {
            ArrayList pieSlicesList = new ArrayList(m_pieSlices);
            PieSlice ps = null;
            ...
            while (incrementIndex < decrementIndex) {
                ...
                Debug.Assert(angle2 >= 0);
                if (angle2 >= angle1) {
                    sliceRight.DrawSides(graphics, bounds);
                    ++incrementIndex;
                }
                else if (angle2 < angle1) {
                    sliceLeft.DrawSides(graphics, bounds);
                    --decrementIndex;
                }					
            }
        ...
        ///   <c>Graphics</c> used for drawing.
        /// </param>
		protected void DrawBottoms(IGUIContext graphics) {
            foreach (PieSlice slice in m_pieSlices) {
