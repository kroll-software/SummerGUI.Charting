...
namespace SummerGUI.Charting.PieChart 
{
	/// <summary>
    ...
    public class PieSlice : IDisposable, ICloneable {
        ...
        /// </summary>
        protected PieSlice(IGUIContext ctx) : base() {
			CTX = ctx;
        }
        ...
        /// </param>
		public PieSlice(IGUIContext ctx, float xBoundingRect, float yBoundingRect, float widthBoundingRect, float heightBoundingRect, float startAngle, float sweepAngle, Color surfaceColor) 
			: this (ctx, xBoundingRect, yBoundingRect, widthBoundingRect, heightBoundingRect, 0F, startAngle, sweepAngle, surfaceColor, ShadowStyle.NoShadow, EdgeColorType.NoEdge) {
        }
        ...
        /// </param>
		public PieSlice(IGUIContext ctx, float xBoundingRect, float yBoundingRect, float widthBoundingRect, float heightBoundingRect, float sliceHeight, float startAngle, float sweepAngle, Color surfaceColor, ShadowStyle shadowStyle, EdgeColorType edgeColorType) 
			: this(ctx) 
		{
        ...
        /// </param>
        public PieSlice(IGUIContext ctx, RectangleF boundingRect, float sliceHeight, float startAngle, float sweepAngle, Color surfaceColor, ShadowStyle shadowStyle, EdgeColorType edgeColorType) 
            : this(ctx, boundingRect.X, boundingRect.Y, boundingRect.Width, boundingRect.Height, sliceHeight, startAngle, sweepAngle, surfaceColor, shadowStyle, edgeColorType) {
        }
        ...
        ///   Edge line width.
        /// </param>
        public PieSlice(IGUIContext ctx, float xBoundingRect, float yBoundingRect, float widthBoundingRect, float heightBoundingRect, float sliceHeight, float startAngle, float sweepAngle, Color surfaceColor, ShadowStyle shadowStyle, EdgeColorType edgeColorType, float edgeLineWidth) 
            : this (ctx, xBoundingRect, yBoundingRect, widthBoundingRect, heightBoundingRect, sliceHeight, startAngle, sweepAngle, surfaceColor, shadowStyle, edgeColorType) {
        ...
        ///   Edge line width.
        /// </param>
		public PieSlice(IGUIContext ctx, Rectangle boundingRect, float sliceHeight, float startAngle, float sweepAngle, Color surfaceColor, ShadowStyle shadowStyle, EdgeColorType edgeColorType, float edgeLineWidth) 
            : this(ctx, boundingRect.X, boundingRect.Y, boundingRect.Width, boundingRect.Height, sliceHeight, startAngle, sweepAngle, surfaceColor, shadowStyle, edgeColorType, edgeLineWidth) {
        }
        ...
        /// </returns>
        public object Clone() {
            return new PieSlice(CTX, BoundingRectangle, SliceHeight, StartAngle, SweepAngle, m_surfaceColor, m_shadowStyle, m_edgeColorType);
        }
        ...
        /// </param>
		public void Draw(IGUIContext graphics, Rectangle bounds) {
            DrawBottom(graphics);
            DrawSides(graphics, bounds);
            DrawTop(graphics);
        }
        ...
        ///   <c>Graphics</c> used to draw the pie slice.
        /// </param>
		internal void DrawSides(IGUIContext graphics, Rectangle bounds) {
            ...
            //DrawVisiblePeriphery(graphics, bounds);
        }
        ...
        internal PieSlice[] Split(float splitAngle) {
			...
			if (Math.Abs(StartAngle - splitAngle) < float.Epsilon || Math.Abs(this.EndAngle - splitAngle) < float.Epsilon)
            ...
            PieSlice pieSlice1 = new PieSlice(CTX, BoundingRectangle, SliceHeight, actualStartAngle, newSweepAngle, m_surfaceColor, m_shadowStyle, m_edgeColorType);
            pieSlice1.InitializeSides(true, false);

            newSweepAngle = GetActualAngle(EndAngle) - splitAngle;
            PieSlice pieSlice2 = new PieSlice(CTX, BoundingRectangle, SliceHeight, splitAngle, newSweepAngle, m_surfaceColor, m_shadowStyle, m_edgeColorType);
        ...
        ///   <c>Graphics</c> used to draw the pie slice.
        /// </param>
		internal void DrawStartSide(IGUIContext graphics) {
            if (m_startSide != null) {
        ...
        ///   <c>Graphics</c> used to draw the pie slice.
        /// </param>
		internal void DrawEndSide(IGUIContext graphics) {
            if (m_endSide != null) {
                // checks if the side is visible 
        ...
        ///   <c>Graphics</c> used to draw the pie slice.
        /// </param>
		internal void DrawVisiblePeriphery(IGUIContext graphics, Rectangle bounds) {
            ...
            foreach (PeripherySurfaceBounds surfaceBounds in peripherySurfaceBounds) {
				/**
				DrawCylinderSurfaceSection(graphics, m_pen, m_brushPeripherySurface, 
					surfaceBounds.StartAngle, surfaceBounds.EndAngle,
					new PointF(bounds.X + surfaceBounds.StartPoint.X, bounds.Bottom - surfaceBounds.StartPoint.Y), 
					new PointF(bounds.X + surfaceBounds.EndPoint.X, bounds.Bottom - surfaceBounds.EndPoint.Y));
				**/

				DrawCylinderSurfaceSection (graphics, m_pen, m_brushPeripherySurface, 
					surfaceBounds.StartAngle, surfaceBounds.EndAngle, surfaceBounds.StartPoint, surfaceBounds.EndPoint);
            }
        }
        ...
        ///   <c>Graphics</c> used to draw the pie slice.
        /// </param>
		internal void DrawHiddenPeriphery(IGUIContext graphics) {
        ...
        ///   <c>Graphics</c> used to draw the pie slice.
        /// </param>
		internal void DrawBottom(IGUIContext graphics) {            
        ...
        ///   <c>Graphics</c> used to draw the pie slice.
        /// </param>
		internal void DrawTop(IGUIContext graphics) {            
			graphics.FillPie(m_brushSurface, m_boundingRectangle, m_startAngle, m_sweepAngle);
        ...
        protected virtual Brush CreateBrushForPeriphery(Color color) {
			...
			LinearGradientBrush brush = new LinearGradientBrush(Color.Blue, Color.White, GradientDirections.Horizontal);
            return brush;
        }
        ...
        /// </param>
		protected void DrawCylinderSurfaceSection(IGUIContext graphics, Pen pen, Brush brush, float startAngle, float endAngle, PointF pointStart, PointF pointEnd) 
		{
        ...
        protected float TransformAngle(float angle) {
			...
			// --- YEAY, that makes it (KROLL)
			double ratio = m_boundingRectangle.Width / m_boundingRectangle.Height;

			double x = m_boundingRectangle.Width * Math.Cos(angle * Math.PI / 180f) / ratio;
			double y = m_boundingRectangle.Height * Math.Sin(angle * Math.PI / 180f);
            float result = (float)(Math.Atan2(y, x) * 180f / Math.PI);
            if (result < 0)
                return result + 360f;
            return result;
        }
        ...
        protected float GetActualAngle(float transformedAngle) {
            double x = m_boundingRectangle.Height * Math.Cos(transformedAngle * Math.PI / 180f);
            double y = m_boundingRectangle.Width * Math.Sin(transformedAngle * Math.PI / 180f);
            float result = (float)(Math.Atan2(y, x) * 180f / Math.PI);
            if (result < 0)
                return result + 360f;
            return result;
        ...
        private void InitializePieSlice(float xBoundingRect, float yBoundingRect, float widthBoundingRect, float heightBoundingRect, float sliceHeight) {
			...
			if (m_sweepAngle % 180 > float.Epsilon)
